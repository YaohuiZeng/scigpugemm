...
int main( void ) {
  ...
  unsigned int nSaltMin, nSaltMax, nSaltStep;
  double cutOff;
  double alpha, beta;
  int iDevice;
  size_t memAllowed;
  ...
  cout << "Enter nSaltMin, nSaltMax, nSaltStep" << endl;
  cin >> nSaltMin >> nSaltMax >> nSaltStep;
  if( nSaltMax <= nSaltMin ) {
    cerr << "Invalid nSalt range" << endl;
    return( EXIT_FAILURE );
  }
  if( nSaltStep < 1 ) {
    cerr << "Invalid nSaltStep" << endl;
    return( EXIT_FAILURE );
  }
  ...
  outputFile << "# m, n, k = " << m << " " << n << " " << k << endl;
  outputFile << "# lda, ldb, ldc = " << lda << " " << ldb << " " << ldc << endl;
  ...
  // ============================================
  // Run a test
  for( unsigned int iSalt=nSaltMin; iSalt<=nSaltMax; iSalt += nSaltStep ) {
